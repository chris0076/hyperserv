...
def openfile(subdir,filename,mode="rb"):
	filename=safepath(config["storage"],os.path.join(subdir,filename))
	filedescriptor=open(filename,mode)
	return filename,filedescriptor

@CSCommand("listfiles")
def listfiles(caller,subdir="."):
	filelist=glob.glob(safepath(config["storage"],os.path.join(subdir,"*")))
	filelist=map(lambda absolute:os.path.split(absolute)[1],filelist)
	return ' '.join(filelist)

@CSCommand("downloadfile","trusted")
@threaded
def downloadfile(caller,address,subdir="maps",filename=None):
	if filename is None:
		filename=posixpath.split(address)[-1]
	source=urllib2.urlopen(address)
	destfilename,destination=openfile(subdir,filename,"wb")
	destination.write(source.read())
	destination.close()
	serverNotice("Downloaded %s to %s" % (address,destfilename))

@CSCommand("deletefile","admin")
def deletefile(caller,subdir,filename):
	filename=safepath(config["storage"],os.path.join(subdir,filename))
	os.remove(filename)
